...
import numpy as np
k = 0
f = open('track_CAS_Pb2+.dat', 'w')
for cas in [4, 6, 8, 10, 12, 14]: 
    mol = gto.Mole()
    mol.atom = [['Pb',(0,0,0)]]
    mol.basis = 'def2-tzvpp'
    mol.ecp = {'Pb':'def2-tzvpp'}
    mol.charge = 2
    mol.build()
    mf = scf.RHF(mol)
    mf.chkfile = 'Pbchk'
...
#    mcc = cc.CCSD(mf).run()
#    et = mcc.ccsd_t()
    f.write(str(cas) + '    ' + str(mf.e_tot) + '    ' + str(cas_e) + '\n')
f.close()
    
#    print(mf.mo_coeff)
#    tools.fcidump.from_mo(mol, 'FCIDUMP.CsI2.def2-tzvpp-'+str(bond_length), mf.mo_coeff)
